...
async function getData(hostnameInput) {
  ...
  try {
    ...
    while (moreResults) {
      ...
      if (newData) {
        ...
        if (newData[0].pages.length < numResults) {
          moreResults = false;
          console.log(
            chalk.bold.green('Success: ') + 'Pulled in data from database'
          );
        }
    ...
    if (data.length) {
      ...
      console.log(
        `Average words per page: ${chalk.blue.bold(averageWordCount)}`
      );
        ...
        .map(
          page =>
            page.links.filter(link =>
              link.product ? link.product.availability === 'AMAZON' : ''
            ).length
        )
        .reduce((acc, curr) => acc + curr);
      console.log(
        `Available product links: ${chalk.green.bold(availableProducts)}`
      );
      ...
      const thirdPartyProducts = data[0].pages
        .map(
          page =>
            page.links.filter(link =>
              link.product ? link.product.availability === 'THIRDPARTY' : ''
            ).length
        )
        .reduce((acc, curr) => acc + curr);
      console.log(
        `3rd party product links: ${chalk.yellow.bold(thirdPartyProducts)}`
      );
      ...
      const unavailableProducts = data[0].pages
        .map(
          page =>
            page.links.filter(link =>
              link.product ? link.product.availability === 'UNAVAILABLE' : ''
            ).length
        )
        .reduce((acc, curr) => acc + curr);
      console.log(
        `Unavailable product links: ${chalk.red.bold(unavailableProducts)}`
      );

      console.log(chalk.bold('\nPercentages'));

      // Display percentage of affiliate links
      const percentAffiliatized = ((affiliateLinks / totalLinks) * 100).toFixed(
        1
      );
      console.log(
        `Affiliate links / Total links: ${chalk.blue.bold(
          percentAffiliatized + `%`
        )}`
      );
      ...
      const percentAvailable = (
        (availableProducts / affiliateLinks) *
        100
      ).toFixed(1);
      console.log(
        `Available product links / Affiliate links: ${chalk.green.bold(
          percentAvailable + `%`
        )}`
      );

      // TODO: Display percentage of third party links
      const percentThirdParty = (
        (thirdPartyProducts / affiliateLinks) *
        100
      ).toFixed(1);
      console.log(
        `3rd party product links / Affiliate links: ${chalk.yellow.bold(
          percentThirdParty + `%`
        )}`
      );

      // TODO: Display percentage of unavailable links
      const percentUnavailable = (
        (unavailableProducts / affiliateLinks) *
        100
      ).toFixed(1);
      console.log(
        `Unavailable product links / Affiliate links: ${chalk.red.bold(
          percentUnavailable + `%`
        )}`
      );
      ...
      fs.writeFile(
        `${hostname.split('.').filter(str => str !== '.')[1]}.csv`,
        csv,
        err => {
          ...
          console.log(
            `${
              hostname.split('.').filter(str => str !== '.')[1]
